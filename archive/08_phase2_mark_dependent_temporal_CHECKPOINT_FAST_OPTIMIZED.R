# Phase 2: Mark-Dependent Temporal Hawkes Models - OPTIMIZED FAST DATASET (VALIDATION)
...
cat("╔═══════════════════════════════════════════════════════════════╗\n")
cat("║  PHASE 2: OPTIMIZED MARK-DEPENDENT HAWKES (FAST VALIDATION) ║\n")
cat("╚═══════════════════════════════════════════════════════════════╝\n\n")
...
cat("FAST DATASET (VALIDATION):\n")
cat("  - Using 1,000 events (6% of data)\n")
cat("  - Max 50 iterations per model\n")
cat("  - Expected time: 15-30 minutes total (2-4 min per model)\n")
cat("  - Validates optimizations before full dataset run\n\n")
...
checkpoint_dir <- "checkpoints_phase2_fast_optimized"
...
cat("Loaded", nrow(marks_data), "events from full dataset\n")
...
marks_data <- marks_data[sample_idx, ]

cat(sprintf("Using sample of %d events (%.1f%% of full dataset)\n",
            n_sample, 100*n_sample/16467))
cat("NOTE: This is for validating optimizations\n")
cat("Full dataset run will follow after validation\n\n")
...
# ====================================================================
# 3. PREPARE SAMPLE DATASET FOR VALIDATION
# ====================================================================

cat("=== PREPARING SAMPLE DATASET FOR VALIDATION ===\n")
cat("Using sampled events for validation\n")

# Use sample dataset (already sampled above)
times_full <- marks_data$time
marks_full <- marks_data

cat(sprintf("Using sample dataset: %d events (%.1f%% of data)\n", nrow(marks_full), 100*nrow(marks_full)/16467))
...
fit_mark_hawkes_optimized <- function(times, marks, time_mat = NULL,
  ...
  # Optimize
  cat("  Starting optimization (max 50 iterations)...\n")
  cat(sprintf("  Expected time: 8-15 minutes (with optimizations)\n"))
...
saveRDS(all_fits, "mark_hawkes_all_models_fast_optimized.rds")
cat("✓ Saved: mark_hawkes_all_models_fast_optimized.rds\n\n")
...
png("plots/21_model_comparison_aic_fast_optimized.png", width = 1200, height = 600, res = 120)
