# ============================================================================
# MODEL C: OFFSET + INTERACTION (Non-Proportional Per-Capita Rates)
# ============================================================================
#
# Specification: μ(district, year) = population × exp(β₀ + δ·log(population) + Σ β_year·I(year))
#
# This model assumes:
# - Population enters as an OFFSET (exposure term)
# - PLUS a log(population) interaction term (δ)
# - Allows per-capita rates to vary with population size
# - If δ = 0: reduces to offset model (constant per-capita rate)
# - If δ < 0: larger districts have LOWER per-capita rates
# - If δ > 0: larger districts have HIGHER per-capita rates
#
# Parameters (15 total):
# - Background (11): β₀, δ, β_2016...β_2024
...
cat("\n")
cat("╔══════════════════════════════════════════════════════════════╗\n")
cat("║   MODEL C: OFFSET + INTERACTION                              ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n")
cat("\n")

cat("SPECIFICATION:\n")
cat("  Background: μ(d,y) = population × exp(β₀ + δ·log(pop) + Σ β_year)\n")
...
cat("KEY ASSUMPTION:\n")
cat("  → Population is EXPOSURE (multiplicative)\n")
cat("  → But per-capita rates can vary with population (δ coefficient)\n")
cat("  → Flexible model: nests both offset (δ=0) and power-law models\n")
cat("  → If δ < 0: larger districts have lower per-capita rates\n")
cat("  → If δ > 0: larger districts have higher per-capita rates\n\n")

cat("PARAMETERS (15 total):\n")
cat("  Background (11): β₀, δ, β_2016...β_2024\n")
...
checkpoint_dir <- "checkpoints_model_c_offset_interact"
...
district_years <- marks_data %>%
  select(district, year, log_pop, population) %>%
  distinct()
...
# ====================================================================
# 3. MODEL C IMPLEMENTATION
# ====================================================================

cat("=== IMPLEMENTING MODEL C (OFFSET + INTERACTION) ===\n\n")

model_c_functions <- function() {
  ...
  # MODEL C: μ(d,y) = population × exp(β₀ + δ·log_pop + Σ β_year)
  calc_mu <- function(population, log_pop, year, params) {
    ...
    # OFFSET + INTERACTION MODEL: population × exp(β₀ + δ·log_pop + year)
    mu <- population * exp(beta_0_bg + delta * log_pop + year_effect)
    return(mu)
  }

  # Conditional intensity
  lambda_offset_interact <- function(idx, times, marks, params, time_mat = NULL) {

    # District-year-specific background rate (OFFSET + INTERACTION MODEL)
    mu_i <- calc_mu(marks$population[idx], marks$log_pop[idx], marks$year[idx], params)
    intensity <- mu_i
  ...
  # Log-likelihood
  loglik_offset_interact <- function(params, times, marks, district_years,
    ...
    for(i in 1:n) {
      lambda_i <- lambda_offset_interact(i, times, marks, params, time_mat)
    ...
    # MODEL C: μ = population × exp(β₀ + δ·log_pop + year_effect)
    for(k in 1:nrow(district_years)) {
      mu_k <- calc_mu(district_years$population[k], district_years$log_pop[k],
                      district_years$year[k], params)
  ...
  return(list(
    ...
    calc_mu = calc_mu,
    lambda = lambda_offset_interact,
    loglik = loglik_offset_interact
  ))
}

hawkes_funcs <- model_c_functions()
...
fit_model_c <- function(times, marks, district_years, time_mat = NULL,
                        model_name = "Model C: Offset + Interaction",
                        model_id = "MODEL_C") {
  ...
  if(!is.null(checkpoint)) {
    ...
    # Initial parameters (11 background params: β₀, δ, 9 years)
    params_init_list <- list(
      # Background parameters (11) - includes delta
      beta_0_bg = log(0.01 / mean(marks$population)),  # Per-capita baseline
      delta = -0.5,  # Initial guess: negative based on diagnostics
      beta_2016 = 0,
      beta_2017 = 0,
      beta_2018 = 0,
      beta_2019 = 0,
...
cat("\n╔══════════════════════════════════════════╗\n")
cat("║   FITTING MODEL C (OFFSET + INTERACT)   ║\n")
cat("╚══════════════════════════════════════════╝\n\n")

model_c_fit <- fit_model_c(
  times_sample,
  marks_sample,
...
cat(sprintf("  AIC: %.2f\n", model_c_fit$AIC))
cat(sprintf("  BIC: %.2f\n", model_c_fit$BIC))
cat(sprintf("  Parameters: %d\n", model_c_fit$n_params))
cat(sprintf("  Runtime: %.2f minutes\n\n", model_c_fit$runtime_mins))

cat("BACKGROUND RATE PARAMETERS:\n")
cat("---------------------------\n")
cat(sprintf("  β₀ (baseline per-capita rate): %.4f\n", model_c_fit$params$beta_0_bg))
cat(sprintf("  δ (population interaction): %.4f\n", model_c_fit$params$delta))

if(abs(model_c_fit$params$delta) < 0.1) {
  cat("    → δ ≈ 0: Suggests constant per-capita rates (offset model adequate)\n")
} else if(model_c_fit$params$delta > 0) {
  cat("    → δ > 0: Larger districts have HIGHER per-capita rates\n")
} else {
  cat("    → δ < 0: Larger districts have LOWER per-capita rates\n")
}
cat("\n")
...
for(yr in 2016:2024) {
  ...
  if(param_name %in% names(model_c_fit$params)) {
    beta_yr <- model_c_fit$params[[param_name]]
...
cat("----------------------\n")
cat(sprintf("  β₀_trig: %.4f\n", model_c_fit$params$beta_0_trig))
cat(sprintf("  β_violence: %.4f (exp = %.3f)\n",
            model_c_fit$params$beta_violence, exp(model_c_fit$params$beta_violence)))
cat(sprintf("  β_state: %.4f (exp = %.3f)\n",
            model_c_fit$params$beta_state, exp(model_c_fit$params$beta_state)))
cat(sprintf("  Decay: %.4f (half-life = %.1f days)\n",
            model_c_fit$params$decay, log(2)/model_c_fit$params$decay))
cat("\n")

# Save results
saveRDS(model_c_fit, "model_c_offset_interact.rds")
cat("✓ Saved: model_c_offset_interact.rds\n\n")

cat("╔══════════════════════════════════════════╗\n")
cat("║   MODEL C ESTIMATION COMPLETE           ║\n")
cat("╚══════════════════════════════════════════╝\n")
