...
double hawkes_negloglik_cpp(
    ...
    NumericVector params,          // Parameter vector
    ...
    // Extract parameters (18 total: 3 background base + 9 year effects + 6 triggering)
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < i; j++) {
            double dt = times[i] - times[j];
            if (dt <= temporal_cutoff && dt > 0) {
                // Alpha for event j (with new marks)
                double alpha = exp(beta_0_trig +
                                  beta_riot * is_riot[j] +
                                  beta_fatal * has_fatalities[j] +
                                  beta_student * is_student[j] +
