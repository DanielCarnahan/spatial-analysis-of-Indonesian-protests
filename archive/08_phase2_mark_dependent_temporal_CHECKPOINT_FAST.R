# Phase 2: Mark-Dependent Temporal Hawkes Models - FAST CHECKPOINT VERSION
# Test hypotheses about violence, fatalities, and state intervention
# This version saves progress after each model and can resume if interrupted
# OPTIMIZED FOR SPEED: Smaller sample, fewer iterations
...
cat("╔═══════════════════════════════════════════════════════════════╗\n")
cat("║    PHASE 2: MARK-DEPENDENT TEMPORAL HAWKES (FAST+CHECKPOINT) ║\n")
cat("╚═══════════════════════════════════════════════════════════════╝\n\n")
...
cat("SPEED OPTIMIZATIONS:\n")
cat("  - Sample: 1,000 events (vs 5,000)\n")
cat("  - Max iterations: 50 (vs 100)\n")
cat("  - Expected time: ~1-2 hours total\n\n")
...
checkpoint_dir <- "checkpoints_phase2_fast"
...
# ====================================================================
# 3. SAMPLE DATA (SMALLER FOR SPEED)
# ====================================================================

cat("=== PREPARING SAMPLE ===\n")

# SMALLER sample for speed
set.seed(123)
n_sample <- min(1000, nrow(marks_data))
...
cat("NOTE: This is a fast exploratory analysis\n")
cat("For publication, use full dataset with more iterations\n\n")

# ====================================================================
# 4. OPTIMIZATION FUNCTION (FASTER)
# ====================================================================
...
fit_mark_hawkes_optim <- function(times, marks, include_violence = FALSE,
  ...
  # Optimize (FASTER: fewer iterations, looser tolerance)
  cat("  Starting optimization (should take 5-10 minutes)...\n")
  ...
  fit <- optim(
    ...
    upper = c(mu = 10, beta_0 = 5, decay = 2,
              beta_violence = 3, beta_fatalities = 3, beta_state = 3)[param_names],
    control = list(
      maxit = 50,           # REDUCED from 100
      trace = 1,
      REPORT = 10,
      factr = 1e7           # LOOSER tolerance (default is 1e7, tighter would be 1e12)
    )
  )
...
# Save comparison
write.csv(comparison_df, "model_comparison_phase2_fast.csv", row.names = FALSE)
cat("✓ Saved: model_comparison_phase2_fast.csv\n\n")
...
write.csv(lr_tests_df, "likelihood_ratio_tests_phase2_fast.csv", row.names = FALSE)
cat("\n✓ Saved: likelihood_ratio_tests_phase2_fast.csv\n\n")
...
saveRDS(all_fits, "mark_hawkes_all_models_fast.rds")
cat("✓ Saved: mark_hawkes_all_models_fast.rds\n\n")
...
png("plots/21_model_comparison_aic_fast.png", width = 1200, height = 600, res = 120)
...
ggplot(comparison_plot, aes(x = reorder(Model_short, AIC), y = AIC)) +
  ...
  labs(title = "Model Comparison: Akaike Information Criterion (AIC)",
       subtitle = "Lower AIC = Better model fit (Fast exploratory analysis)",
       x = "Model",
       y = "AIC") +
  theme_minimal() +
...
dev.off()
cat("✓ Saved: plots/21_model_comparison_aic_fast.png\n")

# Plot 2: Parameter effects (forest plot)
png("plots/22_parameter_effects_forest_fast.png", width = 1000, height = 600, res = 120)
...
dev.off()
cat("✓ Saved: plots/22_parameter_effects_forest_fast.png\n\n")
...
cat("╔═══════════════════════════════════════════════════════════════╗\n")
cat("║           PHASE 2 COMPLETE: FAST EXPLORATORY ANALYSIS        ║\n")
cat("╚═══════════════════════════════════════════════════════════════╝\n\n")

cat("OUTPUTS:\n")
cat("  1. mark_hawkes_all_models_fast.rds - All fitted models\n")
cat("  2. model_comparison_phase2_fast.csv - AIC/BIC comparison\n")
cat("  3. likelihood_ratio_tests_phase2_fast.csv - Hypothesis tests\n")
cat("  4. plots/21_model_comparison_aic_fast.png\n")
cat("  5. plots/22_parameter_effects_forest_fast.png\n")
cat("  6. checkpoints_phase2_fast/ - Individual model checkpoints\n\n")
...
cat("\n\nNOTE: This is a FAST exploratory analysis\n")
cat("  - Sample: 1,000 events (6% of full dataset)\n")
cat("  - Max iterations: 50\n")
cat("  - For publication: Re-run with full dataset and more iterations\n\n")

cat("NEXT STEPS:\n")
cat("  → Review these results to decide if hypotheses are supported\n")
cat("  → If interesting patterns emerge, re-run with full dataset\n")
cat("  → Consider Phase 3 (spatial-temporal ETAS) if needed\n\n")

cat("To clean up checkpoints after successful completion:\n")
cat("  Run: unlink('checkpoints_phase2_fast', recursive = TRUE)\n\n")

cat("=== END PHASE 2 FAST ===\n")
