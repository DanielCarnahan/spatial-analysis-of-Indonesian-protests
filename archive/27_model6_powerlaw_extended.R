#!/usr/bin/env Rscript
# ==============================================================================
# MODEL 6: EXTENDED MARKS HAWKES PROCESS (POWER-LAW KERNEL)
# ==============================================================================
#
...
#
# COMPARISON:
#   - Model 6 vs Model 5: Power-law vs exponential kernel
...
cat("Compiling C++ likelihood function for power-law extended marks Hawkes model...\n")
sourceCpp("hawkes_powerlaw_extended_marks_likelihood.cpp")
cat("✓ C++ compiled\n\n")

cat("\n")
cat("╔══════════════════════════════════════════════════════════════╗\n")
cat("║   MODEL 6: EXTENDED MARKS HAWKES (POWER-LAW)                 ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n")
cat("\n")
...
checkpoint_dir <- "checkpoints_model6_powerlaw_extended"
...
loglik_powerlaw_extended <- function(params, times, marks, district_years,
                                     verbose = TRUE, model_id = NULL) {

  .optim_state$n_evals <- .optim_state$n_evals + 1
  eval_num <- .optim_state$n_evals

  # Prepare parameter vector for C++ (20 parameters)
  param_vec <- c(
    params[["beta_0_bg"]],
    params[["gamma_raw"]],
    params[["delta_raw"]],
    ...
    # Triggering parameters (6 mark effects + 2 kernel)
  ...
  # Call C++ function
  neg_ll <- hawkes_powerlaw_extended_marks_negloglik_cpp(
    times = times,
  ...
  if(!is.null(model_id) && eval_num %% CHECKPOINT_EVERY_N_EVALS == 0) {
    ...
    alpha_display <- transform_alpha(params[["alpha_raw"]])
    save_intermediate_checkpoint(model_id, eval_num, params, ll_final)
    if(verbose) {
      cat(sprintf("\n    💾 Checkpoint %d: LL=%.2f (best: %.2f) | γ=%.4f α=%.2f\n",
                 eval_num, ll_final, .optim_state$best_ll, gamma_display, alpha_display))
    }
  }
  ...
  if(verbose) {
    gamma_display <- transform_gamma(params[["gamma_raw"]])
    alpha_display <- transform_alpha(params[["alpha_raw"]])
    c_display <- transform_c(params[["c_raw"]])
    cat(sprintf("    LL: %.2f | γ=%.4f α=%.2f c=%.3f [eval %d]\n",
               ll_final, gamma_display, alpha_display, c_display, eval_num))
    flush.console()
  }
...
fit_model_powerlaw_extended <- function(times, marks, district_years,
                                        model_name = "Model 6: Power-Law Extended Marks",
                                        model_id = "MODEL6_POWERLAW_EXTENDED",
                                        use_multistart = TRUE,
                                        n_starts = 3) {
    ...
    if(use_multistart && n_starts >= 2) {
      ...
      starting_points[[2]] <- list(
        ...
        params = list(
          ...
          beta_papua_raw = 0.1,
          alpha_raw = alpha2_raw, c_raw = c2_raw
        )
      )
    }

    if(use_multistart && n_starts >= 3) {
      # 3. Lower alpha (slower decay)
      gamma3 <- 0.30; delta3 <- 0.1
      gamma3_raw <- inverse_transform_gamma(gamma3)
      delta3_raw <- inverse_transform_delta(delta3)
      beta_0_trig3 <- log(0.08)
      beta_0_trig3_raw <- inverse_transform_beta_0_trig(beta_0_trig3)
      alpha3 <- 1.5; c3 <- 0.25
      alpha3_raw <- inverse_transform_alpha(alpha3)
      c3_raw <- inverse_transform_c(c3)
      ...
      starting_points[[3]] <- list(
        ...
        params = list(
          ...
          beta_papua_raw = 0.3,
          alpha_raw = alpha3_raw, c_raw = c3_raw
        )
      )
    }
  }
  ...
  lower_bounds <- c(beta_0_bg = -15, gamma_raw = -10, delta_raw = -10,
                    ...
                    beta_labor_raw = -10, beta_papua_raw = -10,
                    alpha_raw = -10, c_raw = -10)
  ...
  obj_fun_silent <- function(par) {
    ...
    ll <- loglik_powerlaw_extended(params_full, times, marks, district_years,
  ...
  for(i in seq_along(starting_points)) {
    ...
    alpha_init <- transform_alpha(start_point$params$alpha_raw)
    cat(sprintf("  Initial: γ=%.3f α=%.2f\n", gamma_init, alpha_init))
    ...
    alpha_result <- transform_alpha(all_results[[i]]$params$alpha_raw)
    cat(sprintf("  → LL: %.2f | γ=%.4f α=%.2f | Conv: %d | Time: %.1f min\n\n",
               all_results[[i]]$loglik, gamma_result, alpha_result,
...
cat("\n╔══════════════════════════════════════════════════════════════╗\n")
cat("║   FITTING MODEL 6: EXTENDED MARKS (POWER-LAW)                ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n\n")
...
model6_fit <- fit_model_powerlaw_extended(
  times_sample,
  marks_sample,
  ...
  n_starts = if(TEST_MODE) 1 else 3
)
...
cat(sprintf("  Runtime: %.2f minutes\n\n", model6_fit$runtime_mins))

cat("BACKGROUND RATE PARAMETERS:\n")
cat("---------------------------\n")
gamma_final <- transform_gamma(model6_fit$params$gamma_raw)
delta_final <- transform_delta(model6_fit$params$delta_raw)

cat(sprintf("  β₀ (baseline): %.4f\n", model6_fit$params$beta_0_bg))
...
cat("------------------------------------------\n")
beta_0_trig_final <- transform_beta_0_trig(model6_fit$params$beta_0_trig_raw)
beta_fatal_final <- transform_beta_mark(model6_fit$params$beta_fatal_raw)
beta_violent_final <- transform_beta_mark(model6_fit$params$beta_violent_raw)
beta_student_final <- transform_beta_mark(model6_fit$params$beta_student_raw)
beta_labor_final <- transform_beta_mark(model6_fit$params$beta_labor_raw)
beta_papua_final <- transform_beta_mark(model6_fit$params$beta_papua_raw)
alpha_final <- transform_alpha(model6_fit$params$alpha_raw)
c_final <- transform_c(model6_fit$params$c_raw)
...
# Save results
saveRDS(model6_fit, "model6_powerlaw_extended.rds")
cat("✓ Saved: model6_powerlaw_extended.rds\n\n")

cat("╔══════════════════════════════════════════════════════════════╗\n")
cat("║   MODEL 6 ESTIMATION COMPLETE                                ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n")
