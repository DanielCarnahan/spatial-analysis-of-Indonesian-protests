# Phase 3: Spatial-Temporal-Mark Hawkes Models - FULL DATASET
# WITH POWER-LAW SPATIAL KERNEL AND HAVERSINE DISTANCES
...
cat("╔═══════════════════════════════════════════════════════════════╗\n")
cat("║  PHASE 3: SPATIAL-TEMPORAL-MARK HAWKES (FULL DATASET)    ║\n")
cat("╚═══════════════════════════════════════════════════════════════╝\n\n")

cat("MODEL SPECIFICATION:\n")
cat("  λ(s,t) = μ + Σ α(marks) · g(d) · exp(-β·τ)\n")
cat("  where:\n")
cat("    g(d) = (1 + d/σ)^(-ω)  [power-law spatial kernel]\n")
...
cat("  - Expected time: ~8-12 hours total\n\n")
...
#SAMPLE_SIZE <- 1000  # Not used in FULL version     # Fast validation

# Early stopping & multi-start
EARLY_STOP_NO_IMPROVE <- 15
EARLY_STOP_MIN_IMPROVE <- 0.1
N_RANDOM_STARTS <- 3
CHECKPOINT_EVERY_N_EVALS <- 10  # Save checkpoint every 10 evaluations
...
checkpoint_dir <- "checkpoints_phase3_full"
...
spatial_temporal_hawkes_functions <- function() {
  ...
  spatial_kernel_powerlaw <- function(d, sigma, omega) {
    # g(d) = (1 + d/σ)^(-ω)
    return((1 + d/sigma)^(-omega))
  }
  ...
  lambda_spatial_temporal <- function(idx, times, lons, lats, marks, params,
    ...
    if(use_mark_interaction) {
      ...
      omega_violent <- params[["omega_violent"]]
      sigma_peaceful <- params[["sigma_peaceful"]]
      omega_peaceful <- params[["omega_peaceful"]]
    }
    ...
    if(idx > 1) {
      ...
      if(length(valid_j) > 0) {
        ...
        if(use_mark_interaction) {
          ...
          if(any(violent_mask)) {
            g_spatial[violent_mask] <- spatial_kernel_powerlaw(dists[violent_mask],
                                                               sigma_violent, omega_violent)
          }
          if(any(!violent_mask)) {
            g_spatial[!violent_mask] <- spatial_kernel_powerlaw(dists[!violent_mask],
                                                                sigma_peaceful, omega_peaceful)
          }
        } else {
          # Same spatial kernel for all
          g_spatial <- spatial_kernel_powerlaw(dists, sigma, omega)
        }
  ...
  loglik_spatial_temporal <- function(params, times, lons, lats, marks,
    ...
    if(use_mark_interaction) {
      ...
      if(is.null(params[["sigma_violent"]]) || is.na(params[["sigma_violent"]]) ||
         is.null(params[["omega_violent"]]) || is.na(params[["omega_violent"]]) ||
         is.null(params[["sigma_peaceful"]]) || is.na(params[["sigma_peaceful"]]) ||
         is.null(params[["omega_peaceful"]]) || is.na(params[["omega_peaceful"]])) {
        return(-Inf)
      }

      if(params[["sigma_violent"]] <= 0 || params[["omega_violent"]] <= 0 ||
         params[["sigma_peaceful"]] <= 0 || params[["omega_peaceful"]] <= 0) {
        return(-Inf)
      }
    } else {
      # Check regular spatial parameters
      if(is.null(params[["sigma"]]) || is.na(params[["sigma"]]) ||
         is.null(params[["omega"]]) || is.na(params[["omega"]])) {
        return(-Inf)
      }

      if(params[["sigma"]] <= 0 || params[["omega"]] <= 0) {
        return(-Inf)
      }
    }
    ...
    for(i in 1:n) {
      if(times[i] < T_max) {
        ...
        # For power-law: integral over circle of radius R ≈ 2π·σ²/(ω-2) for ω > 2
        # Use simplified approximation: mean kernel value × area
        if(use_mark_interaction) {
          if(marks$is_violent[i] == 1) {
            sigma_i <- params[["sigma_violent"]]
            omega_i <- params[["omega_violent"]]
          } else {
            sigma_i <- params[["sigma_peaceful"]]
            omega_i <- params[["omega_peaceful"]]
          }
        } else {
          sigma_i <- params[["sigma"]]
          omega_i <- params[["omega"]]
        }

        # Approximate spatial integral: int_0^R g(d) * 2πd dd
        # For power-law: analytical form exists but complex, use approximation
        # Approximate as: (mean kernel value) × (effective area)
        # Mean kernel value at d=σ: (1 + 1)^(-ω) = 2^(-ω)
        # Effective area: π·σ²
        spatial_integral <- pi * sigma_i^2 * (2^(-omega_i))
    ...
    # Checkpointing
    if(eval_num %% CHECKPOINT_EVERY_N_EVALS == 0) {
      save_intermediate_checkpoint(.optim_state$model_name, eval_num, params, ll_final)
      if(verbose) {
...
fit_spatial_temporal_hawkes <- function(times, lons, lats, marks,
  ...
  if(use_mark_interaction) {
    param_names <- c("mu", "beta_0", "beta_violence", "beta_state", "decay",
                    "sigma_violent", "omega_violent", "sigma_peaceful", "omega_peaceful")
    param_vec <- c(mu = 0.1, beta_0 = -1, beta_violence = 0.5, beta_state = 0.5,
                  decay = 0.2, sigma_violent = 50, omega_violent = 2,
                  sigma_peaceful = 50, omega_peaceful = 2)
  } else if(include_marks) {
    param_names <- c("mu", "beta_0", "beta_violence", "beta_state", "decay", "sigma", "omega")
    param_vec <- c(mu = 0.1, beta_0 = -1, beta_violence = 0.5, beta_state = 0.5,
                  decay = 0.2, sigma = 50, omega = 2)
  } else {
    param_names <- c("mu", "beta_0", "decay", "sigma", "omega")
    param_vec <- c(mu = 0.1, beta_0 = -1, decay = 0.2, sigma = 50, omega = 2)
  ...
  lower_bounds <- c(mu = 1e-6, beta_0 = -10, beta_violence = -10, beta_state = -10,
                   decay = 0.001, sigma = 0.1, omega = 0.01,
                   sigma_violent = 0.1, omega_violent = 0.01,
                   sigma_peaceful = 0.1, omega_peaceful = 0.01)[param_names]
  upper_bounds <- c(mu = 100, beta_0 = 10, beta_violence = 10, beta_state = 10,
                   decay = 5, sigma = 1000, omega = 50,
                   sigma_violent = 1000, omega_violent = 50,
                   sigma_peaceful = 1000, omega_peaceful = 50)[param_names]
  ...
  for(start_i in 2:(N_RANDOM_STARTS + 1)) {
    ...
    random_params["omega"] <- runif(1, 1, 4)
    ...
    if("omega_violent" %in% param_names) random_params["omega_violent"] <- runif(1, 1, 4)
    if("sigma_peaceful" %in% param_names) random_params["sigma_peaceful"] <- runif(1, 20, 100)
    if("omega_peaceful" %in% param_names) random_params["omega_peaceful"] <- runif(1, 1, 4)
...
# Save
write.csv(comparison_df, "model_comparison_phase3_full.csv", row.names = FALSE)
cat("\n✓ Saved: model_comparison_phase3_full.csv\n")
...
write.csv(hypothesis_tests, "hypothesis_tests_phase3_full.csv", row.names = FALSE)
cat("\n✓ Saved: hypothesis_tests_phase3_full.csv\n")
...
spatial_params_df <- data.frame(
  ...
  omega = c(
    results$M1$params["omega"],
    results$M3$params["omega"],
    results$M4$params["omega_violent"],
    results$M4$params["omega_peaceful"]
  )
)

print(spatial_params_df)

write.csv(spatial_params_df, "spatial_parameters_phase3_full.csv", row.names = FALSE)
cat("\n✓ Saved: spatial_parameters_phase3_full.csv\n")

# Interpret spatial decay
cat("\nInterpretation:\n")
cat("  σ (sigma) = characteristic spatial scale (km)\n")
cat("  ω (omega) = spatial decay exponent (higher = faster decay)\n")
cat("  At distance d=σ: triggering drops to 50% of local value\n")
...
saveRDS(results, "spatial_temporal_models_full.rds")
cat("✓ Saved: spatial_temporal_models_full.rds\n")

cat("\n")
cat("╔═══════════════════════════════════════════════════════════════╗\n")
cat("║  COMPLETE - FULL DATASET SUCCESSFUL                        ║\n")
cat("╚═══════════════════════════════════════════════════════════════╝\n")
