# ============================================================================
# MODEL B: POWER-LAW SCALING
# ============================================================================
#
# Specification: μ(district, year) = exp(β₀ + γ·log(population) + Σ β_year·I(year))
#
# This model assumes:
# - Log-log relationship between population and protest counts
# - Population is a COVARIATE that modulates the rate
# - If γ = 1: equivalent to offset model (proportional rates)
# - If γ < 1: larger districts have LOWER per-capita rates
# - If γ > 1: larger districts have HIGHER per-capita rates
#
# Parameters (15 total):
...
cat("KEY ASSUMPTION:\n")
cat("  → Log-log relationship: log(protests) ~ γ·log(population)\n")
cat("  → If γ = 1: proportional (offset model)\n")
cat("  → If γ < 1: sub-proportional (larger districts have lower per-capita rates)\n")
cat("  → If γ > 1: super-proportional (larger districts have higher per-capita rates)\n\n")

cat("PARAMETERS (15 total):\n")
cat("  Background (11): β₀, γ, β_2016...β_2024\n")
...
checkpoint_dir <- "checkpoints_model_b_power_law"
...
cat("=== IMPLEMENTING MODEL B (POWER-LAW) ===\n\n")
...
model_b_functions <- function() {
  ...
  calc_mu <- function(log_pop, year, params) {
    ...
    # POWER-LAW MODEL: log(pop) as covariate
    mu <- exp(beta_0_bg + gamma * log_pop + year_effect)
    return(mu)
  }

  # Conditional intensity
  lambda_power_law <- function(idx, times, marks, params, time_mat = NULL) {
  ...
  # Log-likelihood
  loglik_power_law <- function(params, times, marks, district_years,
    ...
    for(i in 1:n) {
      lambda_i <- lambda_power_law(i, times, marks, params, time_mat)
  ...
  return(list(
    ...
    calc_mu = calc_mu,
    lambda = lambda_power_law,
    loglik = loglik_power_law
  ))
}
...
fit_model_b <- function(times, marks, district_years, time_mat = NULL,
                        model_name = "Model B: Power-Law Scaling",
                        model_id = "MODEL_B") {
  ...
  if(!is.null(checkpoint)) {
    ...
    # Initial parameters (11 background params including gamma)
    params_init_list <- list(
      # Background parameters (11) - includes gamma
      beta_0_bg = log(0.01),
      gamma = 0.5,  # Initial guess for power-law exponent
      beta_2016 = 0,
      beta_2017 = 0,
      beta_2018 = 0,
      beta_2019 = 0,
...
cat(sprintf("  γ (population exponent): %.4f\n", model_b_fit$params$gamma))
cat(sprintf("    → exp(γ) = %.3f\n", exp(model_b_fit$params$gamma)))

if(abs(model_b_fit$params$gamma - 1) < 0.1) {
  cat("    → γ ≈ 1: Suggests proportional relationship (similar to offset model)\n")
} else if(model_b_fit$params$gamma > 1) {
  cat("    → γ > 1: Super-proportional (larger districts have higher per-capita rates)\n")
} else {
  cat("    → γ < 1: Sub-proportional (larger districts have lower per-capita rates)\n")
}
cat("\n")
...
# Save results
saveRDS(model_b_fit, "model_b_power_law.rds")
cat("✓ Saved: model_b_power_law.rds\n\n")
