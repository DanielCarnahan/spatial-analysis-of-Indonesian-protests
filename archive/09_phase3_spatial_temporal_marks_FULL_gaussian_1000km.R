...
cat("  ✓ Spatial cutoff: 1000 km (regional/national scale)\n")
...
SPATIAL_CUTOFF <- 1000   # 1000 km (regional/national scale)
...
checkpoint_dir <- "checkpoints_phase3_full_gaussian_1000km"
  ...
  loglik_spatial_temporal <- function(params, times, lons, lats, marks,
    ...
    for(i in 1:n) {
      if(times[i] < T_max) {
        ...
        # For Gaussian: integral over all space = 2π·σ²
...
fit_spatial_temporal_hawkes <- function(times, lons, lats, marks,
  ...
  if(use_mark_interaction) {
    ...
    param_vec <- c(mu = 0.1, beta_0 = -1, beta_violence = 0.5, beta_state = 0.5,
                  decay = 0.2, sigma_violent = 50, sigma_peaceful = 50)
  ...
  lower_bounds <- c(mu = 1e-6, beta_0 = -10, beta_violence = -10, beta_state = -10,
                   decay = 0.001, sigma = 1,
                   sigma_violent = 1, sigma_peaceful = 1)[param_names]
  upper_bounds <- c(mu = 100, beta_0 = 10, beta_violence = 10, beta_state = 10,
                   decay = 5, sigma = 1000,
                   sigma_violent = 1000, sigma_peaceful = 1000)[param_names]
...
# Save
write.csv(comparison_df, "model_comparison_phase3_full_gaussian_1000km.csv", row.names = FALSE)
cat("\n✓ Saved: model_comparison_phase3_full_gaussian_1000km.csv\n")
...
write.csv(spatial_params_df, "spatial_parameters_phase3_full_gaussian_1000km.csv", row.names = FALSE)
cat("\n✓ Saved: spatial_parameters_phase3_full_gaussian_1000km.csv\n")

# Interpret spatial decay
cat("\nInterpretation:\n")
cat("  σ (sigma) = characteristic spatial scale (km) for Gaussian kernel\n")
cat("  At distance d=σ: triggering drops to exp(-1/2) ≈ 60.6% of local value\n")
...
saveRDS(results, "spatial_temporal_models_full_gaussian_1000km.rds")
cat("✓ Saved: spatial_temporal_models_full_gaussian_1000km.rds\n")

cat("\n")
cat("╔═══════════════════════════════════════════════════════════════╗\n")
cat("║  COMPLETE - FULL DATASET GAUSSIAN 1000KM SUCCESSFUL          ║\n")
cat("╚═══════════════════════════════════════════════════════════════╝\n")
cat("\nResults saved with Gaussian kernel and 1000 km spatial cutoff.\n")
